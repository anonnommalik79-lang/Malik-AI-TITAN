...
const SEEDS: ModeSeed[] = [
  ...
  { id: "reality-check", sectionId: "investor-mode", titleRu: "Проверка на прочность", title: "Reality Check", descriptionRu: "Разбирает готовый план на допущения: чем его можно опровергнуть и при каком результате он мёртв", expertRole: "инвестор, который уже терял деньги на таком же плане, и требует не обещаний, а того, что можно проверить", taskHint: "Разбери план на допущения, назови цифру и дешёвый тест для каждого, и скажи, при каком результате план не работает", outputFormat: "stress" },
