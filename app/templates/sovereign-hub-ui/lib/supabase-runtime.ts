...
declare global {
  interface Window {
    __MALIK_SUPABASE__?: RuntimeSupabaseConfig
  }
}
...
function getWindowRuntimeConfig(): RuntimeSupabaseConfig {
  if (!isBrowser()) return {}
  return window.__MALIK_SUPABASE__ || {}
}

export function getResolvedSupabaseConfig() {
  const runtime = getWindowRuntimeConfig()

  const url = cleanUrl(
    runtime.url ||
      process.env.NEXT_PUBLIC_SUPABASE_URL ||
      (!isBrowser() ? process.env.SUPABASE_URL : ""),
  )
...
function looksLikeSupabaseUrl(value: string) {
  if (!value || value.includes("/rest/v1")) return false
  try {
...
export const isSupabaseConfigured = () => {
  ...
  return Boolean(looksLikeSupabaseUrl(url) && anonKey.length > 20)
}
...
export const getSupabaseDebugInfo = () => {
  ...
  return {
    ...
    urlLooksValid: looksLikeSupabaseUrl(url),
    keyLength: anonKey.length,
    configured: Boolean(looksLikeSupabaseUrl(url) && anonKey.length > 20),
    runtimeConfigPresent: Boolean(getWindowRuntimeConfig().url || getWindowRuntimeConfig().anonKey),
  }
}
...
export const getSupabaseClient = (): SupabaseClient | null => {
  ...
  const { url, anonKey } = getResolvedSupabaseConfig()
  if (!looksLikeSupabaseUrl(url) || anonKey.length <= 20) return null

  const fingerprint = `${url}|${anonKey.slice(0, 16)}|${anonKey.length}`
...
function clearStorageByPredicate(storage: Storage, predicate: (key: string) => boolean) {
...
export const clearSupabaseAuthStorage = (): void => {
  ...
  try {
    clearStorageByPredicate(window.localStorage, shouldClear)
  } catch {}
  try {
    clearStorageByPredicate(window.sessionStorage, shouldClear)
  } catch {}
}
...
export const getAccountRole = (
  ...
  if (!clean || clean === "guest@malik.ai" || (mode === "guest" && !isAdminEmail(clean))) {
    return "guest"
  }
...
export const persistAuthSnapshot = (snapshot: MalikAuthSnapshot): MalikAuthSnapshot => {
  ...
  const normalizedSnapshot = {
    ...snapshot,
    role,
    isAdmin: snapshot.isAdmin || role === "creator" || role === "admin",
  }

  safeLocalStorageSet(STORAGE_KEYS.user, normalizedSnapshot.email)
  safeLocalStorageSet(STORAGE_KEYS.userName, normalizedSnapshot.name)
  safeLocalStorageSet(STORAGE_KEYS.userAvatar, normalizedSnapshot.avatar)
  safeLocalStorageSet(STORAGE_KEYS.authMode, normalizedSnapshot.mode)
  safeLocalStorageSet(STORAGE_KEYS.lastLoginAt, normalizedSnapshot.lastLoginAt)
  ...
  const session = JSON.stringify({
    ok: true,
    user: normalizedSnapshot,
    mode: normalizedSnapshot.mode,
    authenticated: true,
    lastLoginAt: normalizedSnapshot.lastLoginAt,
  })
  ...
  safeLocalStorageSet("malik_guest_unlocked", normalizedSnapshot.mode === "guest" ? "true" : "false")
...
export const getStoredAuthSnapshot = (): MalikAuthSnapshot | null => {
  ...
  const role =
    (safeLocalStorageGet("malik_user_role") as MalikAccountRole) || getAccountRole(email, mode)
  ...
  return {
    ...
    avatar,
    isAdmin:
      safeLocalStorageGet(STORAGE_KEYS.isAdmin) === "true" ||
      role === "creator" ||
      role === "admin",
    role,
    mode,
    lastLoginAt: safeLocalStorageGet(STORAGE_KEYS.lastLoginAt) || "",
  }
}
...
export const subscribeToAuthChanges = (
  callback: (snapshot: MalikAuthSnapshot | null) => void,
) => {
  const supabase = getSupabaseClient()
  if (!supabase) return () => {}
...
export const getAuthProviderStatus = () => ({
  ...
  microsoft:
    ...
    process.env.NEXT_PUBLIC_ENABLE_AZURE_OAUTH === "true",
})
