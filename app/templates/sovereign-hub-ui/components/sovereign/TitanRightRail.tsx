...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <section className={`rounded-[18px] border border-[#d9a928]/14 bg-[#060706] shadow-[inset_0_1px_0_rgba(255,255,255,.025),0_16px_40px_rgba(0,0,0,.18)] ${className}`}>
      {children}
    </section>
  )
}
...
export function TitanRightRail({ chats, activeAiMode, username, onOpenChats, onSelectChat }: TitanRightRailProps) {
  ...
  const modelsPercent = modelCount > 0 ? 100 : 0
  ...
  if (collapsed) {
    return (
      <aside className="hidden w-[52px] shrink-0 flex-col items-center border-l border-[#d9a928]/10 bg-[#020303] py-4 text-white min-[1280px]:flex">
        <button type="button" onClick={toggleCollapsed} aria-label="Развернуть правую панель" className="grid h-9 w-9 place-items-center rounded-xl border border-[#d9a928]/16 bg-[#090a08] text-[#dcb235] transition hover:border-[#dcb235]/40 hover:text-[#ffd65a]">
          <PanelRightOpen className="h-4 w-4" />
        </button>
        <span className={`mt-4 h-2 w-2 rounded-full ${stable ? "bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.75)]" : "bg-amber-400"}`} title={stable ? "Система онлайн" : "Проверка системы"} />
      </aside>
    )
  }
  ...
  return (
    <aside className="hidden w-[300px] shrink-0 flex-col gap-3 overflow-y-auto border-l border-[#d9a928]/10 bg-[#020303] px-3 py-4 text-white min-[1280px]:flex 2xl:w-[314px]">
      <div className="flex items-center justify-between gap-2">
        <button type="button" onClick={toggleCollapsed} aria-label="Свернуть правую панель" className="grid h-9 w-9 place-items-center rounded-xl border border-white/[0.055] bg-[#070807] text-zinc-500 transition hover:border-[#d9a928]/18 hover:text-[#e1b52e]">
      ...
      <Panel>
        <div className="flex items-center justify-between px-4 pb-2 pt-4">
          ...
          <button type="button" onClick={onOpenChats} className="text-[11px] font-semibold text-[#e3b72f] transition hover:text-[#ffd966]">Все</button>
        </div>
        <div className="px-3 pb-3">
          {recentChats.length > 0 ? recentChats.map((chat) => {
            const Icon = iconForChat(chat.title)
            return (
              <button key={chat.id} type="button" onClick={() => onSelectChat?.(chat.id)} className="group flex w-full items-center gap-3 border-b border-white/[0.055] px-1 py-3 text-left last:border-b-0">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[11px] border border-[#d9a928]/20 bg-[#0b0c09] text-[#e1b52e] transition group-hover:border-[#e1b52e]/45 group-hover:bg-[#151208]"><Icon className="h-4 w-4" /></span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[11.5px] font-medium text-zinc-200">{chat.title}</span>
                  <span className="mt-1 flex items-center gap-1 text-[9.5px] text-zinc-600"><Clock3 className="h-3 w-3" />{formatChatTime(chat.timestamp)}</span>
                </span>
              </button>
            )
      ...
      <Panel>
        <div className="flex items-center justify-between px-4 pb-2 pt-4">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#e0b52e]/20 bg-[#b88a12]/10 px-2 py-1 text-[9px] font-semibold text-[#e7bc35]"><span className="h-1.5 w-1.5 rounded-full bg-[#f0c94a]" />Включен</span>
        </div>
      ...
      <Panel>
        <div className="flex items-center justify-between px-4 pb-3 pt-4">
          ...
          <span className={`inline-flex items-center gap-1.5 text-[10px] font-medium ${stable ? "text-emerald-400" : "text-amber-300"}`}><CircleDot className="h-3 w-3" />{stable ? "Онлайн" : "Проверка"}</span>
        </div>
      ...
      <Panel className="relative min-h-[150px] overflow-hidden">
        <div className="relative z-10 max-w-[170px] px-4 py-4">
          ...
          <button type="button" className="mt-5 inline-flex h-8 items-center gap-1 rounded-[9px] border border-[#d9a928]/20 bg-[#171208] px-3 text-[10px] font-semibold text-[#e4ba39] transition hover:border-[#e4ba39]/45 hover:bg-[#211a0a]">Подробнее<ChevronRight className="h-3 w-3" /></button>
        </div>
