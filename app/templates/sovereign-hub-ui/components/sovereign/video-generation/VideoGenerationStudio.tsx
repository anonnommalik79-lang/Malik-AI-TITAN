  ...
  return (
    <main className="mv2" data-view="video-generation-v2" data-phase={phase}>
        ...
        <div className="mv2m__prompt">
          ...
          <div className="mv2m__prompt-foot">
            <div className="mv2m__prompt-tools">
              ...
              <button type="button" onClick={openMobileVideoPicker} aria-label="Загрузить видео до 10 секунд" disabled={busy}><Video /></button>
          ...
          {MODELS.map((model) => {
            ...
            return (
              ...
              >
                ...
                {active ? <span className="mv2__model-selected"><Check /> Выбрано</span> : null}
              </button>
            )
          })}
      ...
      <style jsx>{`
        ...
        .mv2__section-title{display:flex;align-items:center;gap:5px;margin:15px 0 8px;font-size:12px;font-weight:750}.mv2__section-title svg{width:13px;height:13px}.mv2__selected-model{margin-left:auto;max-width:56%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#fff;font-size:9px;font-weight:750}.mv2__models{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.mv2__model{position:relative;min-width:0;height:66px;padding:8px;border:1px solid #272b33;border-radius:11px;background:#0f131a;color:#fff;display:flex;align-items:center;gap:7px;text-align:left;overflow:hidden;transition:border-color .14s ease,background .14s ease,box-shadow .14s ease,transform .14s ease}.mv2__model.is-active{z-index:1;border-color:#fff !important;background:#1d222b !important;box-shadow:0 0 0 2px rgba(255,255,255,.92),inset 0 0 0 1px rgba(255,255,255,.22) !important;transform:translateY(-1px)}.mv2__model:disabled{opacity:.28;cursor:not-allowed}.mv2__model.is-featured{grid-column:1/-1;height:72px;background:#121720}.mv2__model-icon{width:34px;height:34px;flex:0 0 34px;border-radius:8px;background:#fff;display:grid;place-items:center;overflow:hidden}.mv2__model-icon img{width:22px;height:22px;object-fit:contain}.mv2__model-copy{min-width:0;display:flex;flex-direction:column;gap:3px}.mv2__model-copy strong{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:10px}.mv2__model-copy small{color:#858d9b;font-size:8px}.mv2__tier{position:absolute;right:6px;top:5px;padding:2px 5px;border-radius:999px;font-size:7px}.mv2__tier.is-free{background:#fff;color:#000}.mv2__tier.is-pro{background:#252935;color:#c9d0db}.mv2__model-selected{position:absolute;right:7px;bottom:6px;display:inline-flex;align-items:center;gap:3px;height:18px;padding:0 6px;border-radius:999px;background:#fff;color:#050505;font-size:7px;font-weight:850;letter-spacing:.02em}.mv2__model-selected svg{width:9px;height:9px;stroke-width:3}.mv2__model-notice{margin-top:8px;min-height:26px;padding:7px 9px;border:1px solid #2b3038;border-radius:8px;background:#0f1217;color:#c3cad4;font-size:10px}
