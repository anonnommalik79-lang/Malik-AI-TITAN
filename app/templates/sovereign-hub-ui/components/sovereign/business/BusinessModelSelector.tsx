...
export type BusinessModelChoice = {
  ...
  icon: "glm" | "ernie" | "kimi" | "openai" | "qwen"
}

export const BUSINESS_MODEL_CHOICES: readonly BusinessModelChoice[] = [
  { id: "malik-reason-753b", label: "GLM-5.2", providerModel: "ZhipuAI/GLM-5.2", icon: "glm" },
  { id: "malik-core-300b", label: "ERNIE 4.5 300B-A47B-PT", providerModel: "PaddlePaddle/ERNIE-4.5-300B-A47B-PT", icon: "ernie" },
  { id: "malik-flash-53", label: "GLM-5.3", providerModel: "coding-glm-5.3-free", icon: "glm" },
  { id: "malik-vision-k3", label: "Kimi K3", providerModel: "coding-kimi-k3-free", icon: "kimi" },
  { id: "malik-20b", label: "GPT-OSS 20B", providerModel: "openai/gpt-oss-20b", icon: "openai" },
  { id: "malik-fast-120b", label: "GPT-OSS 120B", providerModel: "gpt-oss-120b", icon: "openai" },
  { id: "malik-27b", label: "Qwen3.8-27B", providerModel: "qwen/qwen3.8-27b", icon: "qwen" },
] as const
...
const BUSINESS_PROVIDER_MARKS: Record<BusinessModelChoice["icon"], { label: string; url: string }> = {
  glm: { label: "Z.ai GLM", url: "https://chat.z.ai/favicon.ico" },
  ernie: { label: "Baidu ERNIE", url: "https://ernie.baidu.com/favicon.ico" },
  kimi: { label: "Kimi", url: "https://www.kimi.com/favicon.ico" },
  openai: { label: "OpenAI", url: "https://openai.com/favicon.ico" },
  qwen: { label: "Qwen", url: "https://qwen.ai/favicon.svg" },
}

function ProviderMark({ kind }: { kind: BusinessModelChoice["icon"] }) {
  const brand = BUSINESS_PROVIDER_MARKS[kind]
  return (
    <span className="bm-provider" title={brand.label} aria-label={brand.label}>
      <img src={brand.url} alt="" loading="eager" referrerPolicy="no-referrer" />
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <style jsx global>{`
        ...
        .bm-provider{position:relative;display:grid;width:38px;height:38px;place-items:center;flex:0 0 auto;border:1px solid rgba(255,255,255,.10);border-radius:10px;background:#f7f7f8;overflow:hidden}
        .bm-provider img{display:block;width:25px;height:25px;object-fit:contain}
