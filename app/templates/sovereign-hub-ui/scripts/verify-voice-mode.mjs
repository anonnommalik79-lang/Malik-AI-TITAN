...
const [home, chat, dashboard, sidebar, mode, dock, orb, settings, sound, tts, turn, flux, token, css, homeCss, chatCss] = await Promise.all([
...
const checks = [
  ...
  ["07 microphone uses real audio analyser and noise controls", () => { assert.match(mode, /getUserMedia/); assert.match(mode, /echoCancellation:\s*true/); assert.match(mode, /noiseSuppression:\s*true/); assert.match(mode, /createAnalyser/); assert.match(mode, /getByteFrequencyData/) }],
  ...
  ["09 server STT remains language auto", () => assert.match(mode, /form\.append\("language", "auto"\)/)],
  ...
  ["17 English uses Flux and RU\/KK stay multilingual", () => { assert.match(mode, /language === "en"/); assert.match(tts, /deepgram-flux-batch/); assert.match(tts, /voiceId = "leo"/); assert.match(tts, /language === "ru" \? "ru"/); assert.match(tts, /language === "en" \? "en" : "auto"/) }],
